...
E_Africa <- c("Burundi", "Comoros", "Djibouti", "Eritrea", "Ethiopia", "Kenya", "Madagascar", "Malawi", "Mauritius", "Mozambique", "Rwanda", "Seychelles", "Somalia", "South Sudan", "Uganda","Tanzania", "Zambia", "Zimbabwe")
M_Africa <- c("Angola", "Cameroon", "Central African Republic", "Chad", "Congo", "Dem. Rep. Congo", "Equatorial Guinea", "Gabon", "Sao Tome and Principe", "St Helena" )
...
#Asia
Former_USSR <- c("Former USSR")
E_Asia <- c("China, mainland", "China, Hong Kong SAR", "China, Macao SAR",  "N. Korea", "Japan", "Mongolia", "Republic of Korea")
...
#Europe
E_Europe <- c("Bulgaria", "Former Czechoslovakia", "Hungary", "Poland", "Romania", "Ukraine")
...
sort(unique(shockdrivers$Country))

(synchrony <-
  shockdrivers %>%
  mutate(Time = cut(Year, breaks=10)) %>% 
  ...
  mutate(Country = case_when(Country %in% c("Democratic People's Republic of Korea", "Korea, Dem. People's Rep") ~ "N. Korea",
                             ...
                             T ~ Country),
         SubRegion = case_when(Country %in% N_Africa ~ "North Africa",
           ...
           factor()) %>% 
    mutate(SubRegion = factor(SubRegion, levels(SubRegion)[c(2,4,16,12,11,20,3,18,5,13,22,19,7,8,21,17,6,15,1,9,10, 14)])) %>% 
  arrange(SubRegion))
  



ggplot(data=synchrony, aes(x=Time, y=interaction(Country, SubRegion)))+geom_tile(aes(fill=cut(nn, breaks = 0:4, labels = 1:4), colour="grey25"))+
  scale_fill_manual(drop=F, values = colorRampPalette(c("Yellow", "red1"))(4),na.value="grey90", name="No.of\nsectors")+
  #scale_x_discrete(labels=c("1968-73", "1973-78", "1968-73", "1968-73", "1968-73", "1968-73", "1968-73", "1968-73", "1968-73")) 
  theme_bw()+
  theme(panel.grid = element_blank(),
        axis.text = element_text(size=6))+
  guides(colour=F)+
  ggsave("heatmap.tiff", device = "tiff", dpi=600, width=18, height=15, units = "cm")
  


colorRampPalette(c("Yellow", "red1"))(5)
        
  
 "#FFFF00" "#FFBF00" "#FF7F00" "#FF3F00" "#FF0000"
