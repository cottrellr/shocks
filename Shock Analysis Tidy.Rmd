...
levels(freshfish$Country)[levels(freshfish$Country)=="C魌e d'Ivoire"]<-"Cote d'Ivoire"
freshfish <- freshfish[freshfish$Country!="Saint Helena",]
...
inland$Codes[inland$Country=="R茅union"] <- "REU"
...
marine$Codes[marine$Country=="R茅union"] <- "REU"
...
aggmarineaqua2$Codes[aggmarineaqua2$Country=="R茅union"]<-"REU"
...
agginlandaqua2$Codes[agginlandaqua2$Country=="R茅union"]<-"REU"
...
W_Africa <- c("Benin", "Burkina Faso", "Cabo Verde", "C魌e d'Ivoire", "Gambia", "Ghana", "Guinea", "Guinea-Bissau", "Liberia", "Mali", "Mauritania", "Niger", "Nigeria", "Senegal", "Sierra Leone", "Togo")
...
shocks$Codes[shocks$Country=="R閡nion"] <- "REU"
...
# shockdrivers<-shockdrivers[!(shockdrivers$Sector=="Aquaculture" & shockdrivers$Country=="R茅union"),]
...
N_Africa <- c("Algeria", "Egypt", "Libya", "Morocco", "Sudan", "Tunisia")
E_Africa <- c("Burundi", "Comoros", "Djibouti", "Eritrea", "Ethiopia", "Kenya", "Madagascar", "Malawi", "Mauritius", "Mozambique", "Rwanda", "Seychelles", "Somalia", "South Sudan", "Uganda","Tanzania", "Zambia", "Zimbabwe","United Republic of Tanzania" , "Tanzania, United Rep. of", "Zanzibar", "R閡nion")
M_Africa <- c("Angola", "Cameroon", "Central African Republic", "Chad", "Congo", "Dem. Rep. Congo", "Equatorial Guinea", "Gabon", "Sao Tome and Principe", "St Helena", "Congo, Dem. Rep. of the", "Democratic Republic of the Congo" )
S_Africa <- c("Botswana", "Lesotho", "Namibia", "South Africa", "Swaziland")
W_Africa <- c("Benin", "Burkina Faso", "Cabo Verde", "C魌e d'Ivoire", "Gambia", "Ghana", "Guinea", "Guinea-Bissau", "Liberia", "Mali", "Mauritania", "Niger", "Nigeria", "Senegal", "Sierra Leone", "Togo", "Western Sahara")
...
ggarrange(ggarrange(heat_map, heat_map_country, labels=c("a","b"), nrow=1, ncol=2, legend = F), legend, nrow=2, ncol=1, heights = c(1,0.1), font.label = list(size=12)) +
  ggsave("heatmap3.tiff", device="tiff", dpi=600, width = 18.3, height=15, units="cm")+
  ggsave("heatmap3.pdf", device="pdf", dpi=600, width = 18.3, height=15, units="cm")
...
# Production response in other sectors
...
aggcrops <- 
  ...
  as_data_frame() %>% 
  mutate(Country = case_when(Country %in% c("Democratic People's Republic of Korea", "Korea, Dem. People's Rep") ~ "N. Korea",
                             Country %in% c("Wallis and Futuna Islands", "Wallis and Futuna Is.") ~ "Wallis and Futuna Islands",
                             Country %in% c("Iran (Islamic Republic of)", "Iran (Islamic Rep. of)") ~ "Iran",
                             Country %in% c("Cote d'Ivoire", "C魌e d'Ivoire", "C么te d'Ivoire" ) ~ "Ivory Coast",
...
aggfisheries <- 
  ...
  as_data_frame() %>% 
  mutate(Country = case_when(Country %in% c("Democratic People's Republic of Korea", "Korea, Dem. People's Rep") ~ "N. Korea",
                             Country %in% c("Korea, Republic of", "Republic of Korea") ~ "Republic of Korea",
                             Country %in% c("Wallis and Futuna Islands", "Wallis and Futuna Is.") ~ "Wallis and Futuna Islands",
                             Country %in% c("Iran (Islamic Republic of)", "Iran (Islamic Rep. of)") ~ "Iran",
                             Country %in% c("Cote d'Ivoire", "C魌e d'Ivoire", "C么te d'Ivoire" ) ~ "Ivory Coast",
                             Country %in% c("Venezuela (Bolivarian Republic of)", "Venezuela, Boliv Rep of") ~ "Venezuela",
...
#normalised production data
countries_interest <- unique(shockdrivers$Country)
...
normalised_total <- 
  arrange(bind_rows
          (normalised_crops <- 
              ...
              group_by(Country) %>% 
              nest() %>% 
              mutate(norm_value = map(data, ~normalise(.$Value)))%>% 
              unnest(data, norm_value) %>% 
              filter(Country %in% countries_interest) %>% 
              mutate(Sector = "Crops"),
            
            normalised_livestock <- 
              agglivestock %>% 
              filter(Country %in% countries_interest) %>% 
              mutate(Code = countrycode(Country, origin = "country.name", destination = "iso3c", warn=T)) %>% 
              mutate(SubRegion = case_when(Country %in% N_Africa ~ "North Africa",
            ...
            normalised_fisheries <- 
              aggfisheries %>% 
              filter(Country %in% countries_interest) %>% 
              rename(Code=Codes) %>% 
              select(Country, Year, Value, Continent, Code) %>% 
              mutate(SubRegion = case_when(Country %in% N_Africa ~ "North Africa",
                                           Country %in% M_Africa ~ "Central Africa",
                                           Country %in% S_Africa ~ "Southern Africa",
                                           Country %in% W_Africa ~ "West Africa",
            ...
            normalised_aquaculture <- aggaqua %>% 
              filter(Country %in% countries_interest) %>% 
              rename(Code=Codes) %>% 
              select(Country, Year, Value, Continent, Code) %>% 
              mutate(SubRegion = case_when(Country %in% N_Africa ~ "North Africa",
  ...
  mutate(Code = case_when(Country == "St. Vin.&Gren." ~ "VCT", 
                          Country == "N. Korea" ~ "PRK", 
                           T ~ Code))
            
            

#Find production changes
...
join_df <- function(df1, df2){
  join_together <- left_join(df1, df2, by=c("Code", "Year", "Sector"))
  return(join_together)
}


plus_one_crops <- function(df){
    Country_local <- df$Country
    Year_local <- df$Year+1
    plus_one <- unnested_crops$diff[unnested_crops$Country==Country_local & unnested_crops$Year==Year_local]
    return(plus_one)
  }
...
nested_production <- 
  ...
  group_by(Country) %>% 
  nest() %>%
  mutate(detrended = purrr::map(data, ~resid(loess(data = ., norm_value ~ Year, span=0.6)))) %>% 
  unnest(data, detrended) %>% 
  group_by(Country, Sector) %>% 
  nest() %>% 
  mutate(diff = map(data, ~lag_diff(.$detrended))) %>% 
  unnest(data, diff) %>% 
  group_by(Country) %>% 
  nest() %>% 
  mutate(shocks = map(data, ~ join_df(., shockdrivers))) %>% 
  unnest(shocks) %>% 
  mutate(occurence = case_when(!is.na(Country1) ~ Year)) %>%
  group_by(Country) %>% 
  nest() %>% 
  mutate(Plot = purrr::map(data, ~ggplot(., aes(x=Year, y=diff, colour=Sector ))+geom_line(size=1)+theme_classic()+
                             geom_text(aes(label=unique(Country1)[!is.na(unique(Country1))]), x=1963, y=1, colour="black")+
                             #scale_y_continuous(limits=c(-5,5))+
                             geom_vline(aes(xintercept=occurence, colour=Sector, linetype=Sector)) +
                             scale_color_manual(values = c(Aquaculture ="grey50", Crops = "black", Fisheries = "dodgerblue", Livestock = "firebrick"))+
                             annotate("rect", xmin=-Inf, xmax=Inf, ymin=-1,ymax=1, colour="grey90", alpha=0.1))) 
  


pdf("normalised_prod.pdf", paper="a4r")
invisible(lapply(nested_production$Plot, print))
dev.off()
...
unnested_production <- 
  ...
  mutate(shocks = map(data, ~ join_df(., shockdrivers))) %>% 
  unnest(shocks) %>% 
  mutate(occurence = case_when(!is.na(Country1) ~ TRUE,
                               T ~ FALSE))
...
unnested_crops <- 
  unnested_production %>% 
  filter(Sector =="Crops") %>% 
  select(Code, Year, SubRegion, diff, detrended, occurence)
  
  
unnested_livestock <- 
  unnested_production %>% 
  filter(Sector =="Livestock")%>% 
  select( Code, Year, SubRegion, diff, detrended, occurence)
...
unnested_aquaculture <- 
  unnested_production %>% 
  filter(Sector =="Aquaculture")%>% 
  select( Code, Year, SubRegion, diff, detrended, occurence)


#join crop shocks to normalised production

crop_shocks <- 
  shockdrivers %>% 
  filter(Sector=="Crops") %>% 
  left_join(., unnested_crops, by=c("Code", "Year")) %>% 
  select(Sector, Country, Year, Region, SubRegion, Code, AbsMagnitude, Recovery2, Category, diff) %>% 
  rename(Crop_change = diff) %>% 
  left_join(., unnested_livestock, by=c("Code", "Year")) %>% 
  select(Sector, Country, Year, Region, SubRegion.x, Code, AbsMagnitude, Recovery2, Category, Crop_change, diff) %>% 
  rename(LS_change =diff) %>% 
  left_join(., unnested_fisheries, by=c("Code", "Year")) %>% 
  select(Sector, Country, Year, Region, SubRegion.x, Code, AbsMagnitude, Recovery2, Category, Crop_change, LS_change, diff) %>% 
  rename(Fish_change = diff) %>% 
  left_join(., unnested_aquaculture, by=c("Code", "Year")) %>% 
  select(Sector, Country, Year, Region, SubRegion.x, Code, AbsMagnitude, Recovery2, Category, Crop_change, LS_change, Fish_change, diff) %>% 
  rename(Aqua_change = diff) %>% 
  mutate(crop_response = case_when(Crop_change>=0.5 ~ "Increase",
                                   Crop_change<= -0.5 ~ "Decrease",
                                   T ~ "No change")) %>% 
  mutate(LS_response = case_when(LS_change>=0.5 ~ "Increase",
                                 LS_change<= -0.5 ~ "Decrease",
                                 T ~ "No change")) %>% 
  mutate(fish_response = case_when(Fish_change>=0.5 ~ "Increase",
                                   Fish_change<= -0.5 ~ "Decrease",
                                   T ~ "No change")) %>% 
  mutate(aqua_response = case_when(Aqua_change>=0.5 ~ "Increase",
                                   Aqua_change<= -0.5 ~ "Decrease",
                                   T ~ "No change"))
         

# rows <-  
#   split(crop_shocks, seq(nrow(crop_shocks)))
# 
# crop_prod_plus1 <- lapply(rows, function(target_row){
#   Code_local <- target_row$Code
#   Year_local <-  target_row$Year +1
#   Crop_changeplus1 <- unnested_crops$diff[unnested_crops$Code==Code_local & unnested_crops$Year==Year_local]
#   LS_changeplus1 <- unnested_livestock$diff[unnested_livestock$Code==Code_local & unnested_livestock$Year==Year_local]
#   Fish_changeplus1 <- unnested_fisheries$diff[unnested_fisheries$Code==Code_local & unnested_fisheries$Year==Year_local]
#   Aqua_changeplus1 <- unnested_aquaculture$diff[unnested_aquaculture$Code==Code_local & unnested_aquaculture$Year==Year_local]
#   return(Fish_changeplus1)
# })
# 
# 
# crop_prod_plus1[[31]]
...
#join livestock shocks to normalised production 
...
livestock_shocks <- 
  ...
  select(Sector, Country, Year, Region, SubRegion.x, Code, AbsMagnitude, Recovery2, Category, Crop_change, LS_change, Fish_change, diff) %>% 
  rename(Aqua_change = diff) %>% 
  mutate(crop_response = case_when(Crop_change>=1 ~ "Increase",
                                   Crop_change<= -1 ~ "Decrease",
                                   T ~ "No change")) %>% 
  mutate(LS_response = case_when(LS_change>=1 ~ "Increase",
                                 LS_change<= -1 ~ "Decrease",
                                 T ~ "No change")) %>% 
  mutate(fish_response = case_when(Fish_change>=1 ~ "Increase",
                                   Fish_change<= -1 ~ "Decrease",
                                   T ~ "No change")) %>% 
  mutate(aqua_response = case_when(Aqua_change>=1 ~ "Increase",
                                   Aqua_change<= -1 ~ "Decrease",
                                   T ~ "No change"))


# rows <-  
#   split(livestock_shocks, seq(nrow(livestock_shocks)))
# 
# 
# 
# 
...
#join fisheires shocks to normalised production
...
mean(crop_shocks$LS_change[!is.na(crop_shocks$LS_change)])
sd(crop_shocks$LS_change[!is.na(crop_shocks$LS_change)])
mean(crop_shocks$Fish_change[!is.na(crop_shocks$Fish_change)])
sd(crop_shocks$Fish_change[!is.na(crop_shocks$Fish_change)])
mean(crop_shocks$Aqua_change[!is.na(crop_shocks$Aqua_change)])
sd(crop_shocks$Aqua_change[!is.na(crop_shocks$Aqua_change)])

```

#Random forest - what determines if multiple shocks occur with a given period

```{r}

#Using possible predictor variables of why multiple shocks co-occur

###Hypotheses
# HDI (development level) - more developed countries maybe able to buffer mutliple shocks through subsidies or other economic tools - and less developed nations tend to have a greater dependence on food production for livelihoods
# Category of driver - some drivers may be more prone to affect multiple sectors in general (e.g. economic turndown)
#SubCategory of driver - more specificity may help the predictive power of the above
# Size of the shock - shocks that are bigger may be from drivers that may affect multiplpe sectors and be more flexible than rigid categories assigned to drivers
#Recovery time - for shocks with long recovery times, these may represent step changes in the food system, such as the USSR dissolution or shifts of pressure between sectors e.g Brashares 2004
#Region - some regions are more shock prone than others 
#Subregion - as above but with more specificity - shocks may spillover into neighbouring countries
#Land-locked - war and droughts are more likely to move across multiple sectors if a country is dependent on inland systems for aquatic food too - compared to the buffering provided by marine systems
#Land-area - in smaller countries one shock from war or drought may impact multiple sectors such as crops, livestock and fisheries or aquaculture



#HDI data

HDI <- fread("Human development index (HDI) (1).csv", header=T) %>%
  gather(key = Year, value=HDI, -c(`HDI Rank (2015)`, Country)) %>%
  arrange(Country) %>%
  mutate(Codes = countrycode(Country, origin = "country.name", destination = "iso3c", warn=T)) %>%
  mutate(Year = as.integer(Year))


#fill in HDI NAs from 1990 onwards wirth minimum NAs

HDIcountries <- HDI$Codes
HDI_Countries <- split(unique(HDI$Country), unique(HDI$Country))

HDI_codes <- (lapply(HDI_Countries, function(target_country){
  hdi_local <- HDI$HDI[HDI$Country==target_country]
  hdi_local[is.na(hdi_local)] <- min(hdi_local[!is.na(hdi_local)])
  return(hdi_local)
})
)

HDI <- HDI %>%
  mutate(HDI = unlist(HDI_codes))

shock_countries_list <- split(shockdrivers, seq(nrow(shockdrivers)))

#Create vector that assigns lowest HDI values for shocks before 1990 (HDI data start) 
HDIs <- unlist(lapply(shock_countries_list, function(target_shock){
  Year <- target_shock$Year
  Codes <- target_shock$Codes
  if (Codes %in% HDIcountries & Year %in% 1990:2015){
    HDI <- HDI$HDI[HDI$Codes==Codes & HDI$Year==Year]
    return(HDI)
  } else if (Codes %in% HDIcountries & !(Year %in% 1990:2015)){
    HDI <- min(HDI$HDI[HDI$Codes==Codes])
    return(HDI)
  } else {
    HDI <- NA
    return(HDI)
...
#land area data
land_area <- fread("land_area.csv", header=T) %>% 
  rename(Country=`Country Name`,
         Codes = `Country Code`,
         Indicator = `Indicator Name`,
         Ind_code = `Indicator Code`) %>% 
  select(-c(Indicator, Ind_code, `1960`)) %>% 
  gather(key=Year, value=Area, -c(Country, Codes)) %>% 
  mutate(Year = as.integer(Year)) %>% 
  arrange(Country)
  

land_locked <- fread("landlockedness.csv", header=T) %>% 
  select(Country) %>% 
  mutate(Codes = countrycode(Country, origin= "country.name", destination = "iso3c", warn=T)) %>% 
  na.omit()

#Countries where double/triple jeopardies happened
double_countries <-  unique(synchrony$Country[synchrony$nn>1])
double_codes <- countrycode(double_countries, origin = "country.name", destination = "iso3c", warn=T)
double_codes[is.na(double_codes)] <- "VIN"




shockdrivers2 <-  shockdrivers %>% 
  select(-c(PropMagnitude, Recovery,Driver, Reference,Scaled.recovery)) %>% 
  mutate(Land_locked = case_when(Codes %in% land_locked$Codes ~ "Yes", 
                                 T ~ "No")) %>% 
  left_join(land_area, by=c("Codes", "Year")) %>% 
  select(-Country.y) %>% 
  mutate(Area = case_when(Country.x=="Guadeloupe" ~ 1628,
                          Country.x=="Niue" ~ 261.5,
                          Country.x=="Tokelau" ~ 10,
                          Country.x=="Wallis and Futuna Islands" ~ 142.2,
                          Country.x=="Former Czechoslovakia" ~ 127900,
                          Country.x=="Anguilla" ~ 91,
                          Country.x=="Belgium" ~ 30528,
                          Country.x=="Cook Islands" ~ 236.7,
                          Country.x=="Mayotte" ~ 374,
                          Country.x=="Netherlands Antilles" ~ 999,
                          Country.x=="Palau" ~ 459,
                          Country.x=="St Helena" ~ 121.7,
                          Country.x=="St. Pierre and Miquelon" ~ 242,
                          Country.x=="French Guiana" ~ 83534,
                          Country.x=="Martinique" ~ 1128,
                          Country.x=="Montserrat" ~ 102,
                          T ~ Area)) %>% 
  mutate(HDI = HDIs) %>% 
  mutate(HDI_class = case_when(HDI>=0.8~"Very high",
                               HDI>=0.7 & HDI<0.8 ~ "High",
                               HDI>=0.55 & HDI<0.7 ~ "Medium",
                               HDI<0.55~"Low",
                               is.na(HDI)~"Unclassified")) %>% 
  select(-HDI) %>% 
  mutate(Multiple_sectors = case_when(Codes %in% double_codes ~ "Yes",
                                      T ~ "No")) %>%
  mutate(Multiple_sectors = case_when(Country.x == "N. Korea" & Year <1993 ~ "No",
                                      Country.x == "Maldives" & Year <2003 ~ "No",
                                      Country.x == "Iraq" & Year <2008 ~ "No",
                                      Country.x == "Burundi" & Year <2008 ~ "No",
                                      Country.x == "Hungary" & Year <1998 ~ "No",
                                      T~ Multiple_sectors)) %>% 
...
#split data frame keeping equal distribution of multi sector instances 
set.seed(90)
split <- stratified(shockdrivers2, "Multiple_sectors", size=0.5, replace=FALSE, bothSets=TRUE)
training_set <- arrange(split[[1]])
valid_set <- split[[2]]

#Establish importance variables through cForest
expl_variables <- paste(names(shockdrivers2)[-c(1,2,6,14)], collapse = "+")
formula <- as.formula(paste(names(shockdrivers2)[14], expl_variables, sep="~"))
set.seed(90)
rfobject <- cforest(formula = formula, data = training_set, controls =   cforest_control(mtry = 3, ntree = 500,replace = F,teststat = "quad",testtype = "Univ", mincriterion = 0))
varimp_cforest <-  as_vector(sort(varimp(rfobject)))
importance <- data_frame(Variable=names(varimp_cforest), Perm_importance = varimp_cforest)
ggplot(importance, aes(x=reorder(Variable, Perm_importance), y=Perm_importance))+geom_point()



expl_variables <- paste(names(shockdrivers2)[c(10,4,7)], collapse = "+")
formula <- as.formula(paste(names(shockdrivers2)[14], expl_variables, sep="~"))
(rf_classifier = randomForest(formula, data = training_set, ntree=500, type="classification", importance=T))
prediction <- predict(rf_classifier, valid_set[,-14])
table(observed=valid_set$Multiple_sectors,predicted=(prediction))
match <- (prediction == valid_set$Multiple_sectors)
length(match[match==FALSE])/length(prediction)
...
exp(5)
#subregion #category #land.area #sector #shock.size #recovery.time
...
ratescombined<-ggarrange(mcrops, mLs, mfish, mAqua, ncol = 1, nrow = 4, labels = c("a", "c", "e", "g"), legend = "none", font.label = list(size=12)) # pair this with temporal trends
...
ggsave("Drivers only.tiff", device="tiff", dpi=600, width=8.9, height=14, units = "cm")+
ggsave("Drivers only.pdf", device= "pdf", dpi=600, width=8.9, height=14, units = "cm")
...
time_rates <- ggarrange(p1,p3,p5,p7, labels = c("b", "d", "f", "h"), nrow = 4, ncol=1, legend = "none", heights=c(1,1,1,1.35), font.label = list(size=12))
...
ggarrange(ratescombined2, time_rates, nrow = 1, ncol=2)+
  ggsave("shocks rates spatial temporal.tiff", device = "tiff", dpi=600, width = 18.3, height=20, units="cm")+
  ggsave("shocks rates spatial temporal.pdf", device = "pdf", dpi=600, width = 18.3, height=20, units="cm")
...
cols2<-c("Fisheries landings" ="dodgerblue", "Crop production"="firebrick", "Livestock production"="black", "Aquaculture production")
...
ggsave(filename = "Tradeoffs and synergies2.tiff", device = "tiff", dpi = 600, width=18.3, height=12, units = "cm")+
ggsave(filename = "Tradeoffs and synergies2.pdf", device = "pdf", dpi = 600, width=18.3, height=12, units = "cm")
...
shockdrivers2$Country[shockdrivers2$Country=="R茅union"]<-"Reunion"
