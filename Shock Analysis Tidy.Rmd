...
#Random forest - what determines mutliple shocks in a given period
...
#Random forest
expl_variables <- paste(names(shockdrivers2)[-c(1,2,6,12,14)], collapse = "+")
...
#keep only subcategory & area 
expl_variables <- paste(names(shockdrivers2)[c(7,10)], collapse = "+")
formula <- as.formula(paste(names(shockdrivers2)[14], expl_variables, sep="~"))
(rf_classifier = randomForest(formula, data = training_set, ntree=500, type="classification", importance=T))
varImpPlot(rf_classifier)
prediction <- predict(rf_classifier, valid_set[,-14])
...
#only area
expl_variables <- paste(names(shockdrivers2)[c(12)], collapse = "+")
formula <- as.formula(paste(names(shockdrivers2)[14], expl_variables, sep="~"))
(rf_classifier = randomForest(formula, data = training_set, ntree=500, type="classification", importance=T))
prediction <- predict(rf_classifier, valid_set[,-14])
table(observed=valid_set$Multiple_sectors,predicted=(prediction))
...
shockdrivers2 %>% 
  group_by(HDI_class, Multiple_sectors) %>% 
  count() %>% 
  ggplot(aes(x=HDI_class, y=n, fill=Multiple_sectors))+geom_bar(stat="identity",position = "dodge")
...
prediction_for_table <- predict(rf_classifier,val_set[,-5])
table(observed=validation1[,5],predicted=prediction_for_table)

library(ROCR) # roc curve and compute the area underneath
