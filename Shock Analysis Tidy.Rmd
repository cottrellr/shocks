...
# Size of the s hock - shocks that are bigger may be from drivers that may affect multiplpe sectors and be more flexible than rigid categories assigned to drivers
...
S1 <- ggplot(importance, aes(x=reorder(Variable, Perm_importance), y=Perm_importance))+
  geom_point()+
  theme_bw()+
  theme(panel.grid = element_blank(),
  ...
  labs(y=bquote(bold(Permutation~importance)), x="")+
  scale_x_discrete(labels = c("Shock size", "Landlockedness", "Subregion", "HDI", "Region", "Country Area", "Recovery time","Shock type", "Shock subtype", "Year"))+
  ggsave("Figure S2a.tiff", device = "tiff", dpi = 300, width=8.9, height=8, units = "cm")
...
mytable <- table(Observed=valid_set$Multiple_sectors,Predicted=(prediction))
match <- (prediction == valid_set$Multiple_sectors)
length(match[match==FALSE])/length(prediction)

S2 <- ggplot(as.data.frame(mytable), aes(x=Observed, y=Freq, fill=Predicted))+
  ...
  labs(x=bquote(bold(Multi~sector~shocks~observed)), y=bquote(bold(Frequency)))+
  scale_fill_manual(values=c( "grey40", "dodgerblue"))+
  ggsave("Figure S2b.tiff", device = "tiff", dpi=300, width = 8.9, height = 8, units = "cm")

ggarrange(S1, S2, labels=c("a", "b"), nrow = 1, ncol = 2, font.label = list(size=12))+
  ggsave("Figure S2.tiff", device = "tiff", dpi=600, width = 18.3, height = 8, units = "cm")



write.csv(shockdrivers2, "Adapated Shock data.csv", row.names = F)
...
#Figure 1 - Shocks over time plot
...
ggarrange(ggarrange(p2,p4,p6,p8, labels = c("a", "b", "c","d"), nrow=2, ncol=2, font.label = list(size=12), legend = "none"), legend, nrow = 2, ncol=1, heights = c(1,0.2))+
  ggsave("Figure S3.tiff", device = "tiff", dpi=300, width = 18.3, height = 14, units="cm")
...
# Old code - Production response in other sectors
...
aggcrops <- 
  ...
  mutate(Country = case_when(Country %in% c("Democratic People's Republic of Korea", "Korea, Dem. People's Rep") ~ "N. Korea",
                             ...
                             Country %in% c("Cote d'Ivoire", "Côte d'Ivoire", "CÃ´te d'Ivoire" ) ~ "Ivory Coast",
