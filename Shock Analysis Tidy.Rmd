...
aggaqua$Continent[aggaqua$Country=="Réunion"]<-"Africa"
...
# agginlandaqua$Continent[agginlandaqua$Country=="Réunion"]<-"Africa"
# 
# 
# #SubSaharan Africa
...
# aggmarineaqua$Continent[aggmarineaqua$Country=="Réunion"]<-"Africa"
...
p1<-ggplot(data=Shocks.df, aes(x=D, y=CropsNormal))+geom_bar(stat = "identity")+
  labs(x="", y="No. of shocks detected", title = "Crops")+
  theme_classic()+
  theme(panel.grid = element_blank(),
...
grid<-ggarrange(toprow, secondrow, thirdrow, bottomrow, ncol=1, nrow = 4, align = "h")
...
shockdrivers <- read.csv("shocksanddrivers.csv", header=T)
shockdrivers <- shockdrivers[1:230,1:11]
...
unique(shockdrivers$Region)
unique(shockdrivers$Country[shockdrivers$Region=="EuCA"])
...
shockdrivers$Category<-factor(shockdrivers$Category,levels(shockdrivers$Category)[c(2,1,3, 4, 5, 6, 7, 9, 8, 10)])
levels(shockdrivers$Category)
...
write.csv(shockdrivers, file = "Shocks and drivers tidy.csv", row.names = F)

```

#PLOTS FOR PROPORTIONAL CONTRIBUTION OF DRIVERS PER SECTOR

```{r}
cols<-c(terrain_hcl(9, h = c(250, 40), c = 96, l = c(65, 90)),"grey60")
...
size1<-ggplot(data = shockdrivers[shockdrivers$Sector=="Crops",], aes(x=Region, y=log(AbsMagnitude)))+geom_boxplot(outlier.alpha = 0)+
  geom_jitter(aes(colour=Category), size=1.5, alpha=0.5, width=0.2)+
  ...
  geom_text(data = maxshockscrops, mapping = aes(x=Region, y= log(AbsMagnitude), label = Country), size=3, position = position_nudge(y = 1), hjust="inward")
...
size2<-ggplot(data = shockdrivers[shockdrivers$Sector=="Livestock",], aes(x=Region, y=log(AbsMagnitude)))+geom_boxplot(outlier.alpha = 0)+
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.text.x = element_blank())+
  labs(y="log(Shock size (T))")+
  geom_text(data = maxshockslivestock, mapping = aes(x=Region, y= log(AbsMagnitude), label = Country), size=3, position = position_nudge(y = 1), hjust="inward")
...
size3<-ggplot(data = shockdrivers[shockdrivers$Sector=="Fisheries",], aes(x=Region, y=log(AbsMagnitude)))+geom_boxplot(outlier.alpha = 0)+
  ...
  theme(panel.grid = element_blank(),
        ...
        plot.margin = unit(c(0,0.2,0,0), "cm"))+
  labs(y="log(Shock size (T))")+
  geom_text(data = maxshocksfisheries, mapping = aes(x=Region, y= log(AbsMagnitude), label = Country), size=3, position = position_nudge(y = 1), hjust="inward")
...
size4<-ggplot(data = shockdrivers[shockdrivers$Sector=="Aquaculture",], aes(x=Region, y=log(AbsMagnitude)))+geom_boxplot(outlier.alpha = 0)+
  ...
  geom_text(data = maxshocksaqua, mapping = aes(x=Region, y= log(AbsMagnitude), label = Country), size=3, position = position_nudge(y = 1), hjust="inward")
...
allnl<-ggarrange(row1, row2, row3, row4, nrow = 4, ncol = 1, legend = "none", heights = c(1.05,1, 1,1.5))

ggsave("drivers combined.test2.tiff", device = "tiff", dpi = 600, width = 17.3, height = 18, units = "cm")

driverlegend<-get_legend(pie1)

ggarrange(allnl, driverlegend, nrow=2, ncol=1, heights = c(1,0.15))

ggsave(filename = "Drivers2.tiff", device="tiff", dpi=600, width=16.3, height=20, units="cm")
...
unique(aquacultureSR$Rate[aquacultureSR$Continent=="Latin America and Caribbean"])/unique(aquacultureSR$Rate[aquacultureSR$Continent=="Europe and Central Asia"])
...
##Composite exposure indices excluding the fall of communism
...
shockdrivers2$Country[shockdrivers2$Country=="Réunion"]<-"Reunion"
...
#Magnitude and recovery time plot
...
for (i in 1:nrow(av.shocks)){
  if (av.shocks$Year[i]>1959 & av.shocks$Year[i] < 1970){
    av.shocks$decade[i] <- "1960s"
  } else if(av.shocks$Year[i]>1969 & av.shocks$Year[i] < 1980){
    av.shocks$decade[i] <- "1970s"
  } else if(av.shocks$Year[i]>1979 & av.shocks$Year[i] < 1990){
    av.shocks$decade[i] <- "1980s"
  } else if(av.shocks$Year[i]>1989 & av.shocks$Year[i] < 2000){
    av.shocks$decade[i] <- "1990s"
  } else if(av.shocks$Year[i]>1999){
    av.shocks$decade[i] <- "2000s"
  } else if(av.shocks$Year[i]>2009 & av.shocks$Year[i] < 2020){
    av.shocks$decade[i] <- "2010s"
  }
}
...
(p2 <-ggplot(data=cropdriversovertime, aes(x=Year, y=Freq, fill=Driver))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=cols)+
  theme_bw()+
  scale_x_continuous(limits = c(1968, 2013))+
  scale_y_continuous(limits = c(0, 8))+
  theme(panel.grid = element_blank(),
        axis.text = element_text(size=8),
        axis.title = element_text(size=8),
        axis.title.x = element_blank())+
  labs(y=bquote(bold(Shock~freq.))))
...
(p4 <-ggplot(data=livestockdriversovertime, aes(x=Year, y=Freq, fill=Driver))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=cols)+
  theme_bw()+
  scale_x_continuous(limits = c(1968, 2013))+
...
(p6 <-ggplot(data=fisheriesdriversovertime, aes(x=Year, y=Freq, fill=Driver))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=cols)+
  theme_bw()+
  scale_x_continuous(limits = c(1968, 2013))+
...
(p8 <-ggplot(data=aquaculturedriversovertime, aes(x=Year, y=Freq, fill=Driver))+
  geom_bar(stat="identity")+
  scale_fill_manual(values=cols)+
  theme_bw()+
  scale_x_continuous(limits = c(1968, 2013))+
  ...
  labs(y=bquote(bold(Shock~freq.)), x=bquote(bold(Year)))+
...
fig1<-ggarrange(p1, p2, p3,p4, p5,p6, p7,p8, labels=c("a","b","c","d","e", "f", "g", "h"), ncol=2, nrow=4, heights = c(1,1,1,1.1), legend = "none")
...
#MAPPING SHOCK MAGNITUDES and DRIVERS
...
#LAND_SEA SWITCHES OR SYNCHRONIES 
...
ggpubr::ggarrange(p9,p10, labels=c("a", "b"), ncol=2, nrow = 1)
ggplot2::ggsave(filename = "Tradeoffs and synergies2.pdf", device = pdf, dpi = 600, width=16, height=6, units = "cm")
