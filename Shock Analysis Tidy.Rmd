...
length(unique(synchrony$Country[synchrony$SubRegion=="Caribbean" ]))
...
heat_map <- ggplot(data=synchrony, aes(x=Time, y=interaction(Country, SubRegion)))+
  geom_tile(aes(fill=cut(nn, breaks = 0:3, labels = 1:3)), colour="black")+
  geom_hline(yintercept = Sregion_nos2,  alpha=0.4, linetype=3)+
  scale_fill_manual( values = colorRampPalette(c("Yellow", "Red1"))(3), na.value="salmon")+
...
annotate_figure(heat_map, left =text_grob("Polynesia\nMicronesia\nMelanesia\nAustralia/NZ\nSE Asia\nEast Asia\nSouth Asia\nWestern Asia\nFormerUSSR\nEastern Europe\n", vjust = 0.5, color="black", size=7))


labels <-as.vector(rev(unique(synchrony$SubRegion)))
