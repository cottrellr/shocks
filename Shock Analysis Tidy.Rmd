...
#Random forest - what determines mutliple shocks in one year period
...
#Countries where double jeopardies happened
double_countries <-  unique(synchrony$Country[synchrony$nn>1])
...
shockdrivers2 <-  shockdrivers %>% 
  ...
  mutate(Multiple_sectors = case_when(Codes %in% double_codes ~ "Yes",
                                      T ~ "No")) %>% 
  mutate_if(sapply(shockdrivers2, is.character), as.factor)
...
sample_size <-  floor(nrow(shockdrivers2)/2)
indices <- sample(1:nrow(shockdrivers2), size=sample_size)

#separate training and validation set

training_set <- shockdrivers2[indices,]
val_set <- shockdrivers2[-indices,]
...
#perform training
rf_classifier = randomForest(Multiple_sectors~SubRegion+Area, data = training_set, ntree=500, mtry=2)
